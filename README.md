...
- **差分法距离修正** (可选): MOG2 背景建模 + 轮廓实心填充，从基准帧（无人体）与起跳/落地帧的脚部 ROI 精确提取鞋子边缘位置，减少不同鞋子尺寸带来的误差
...
### DiffDetector 差分法（可选，默认开启）

差分法作为骨骼法的补充测量，利用无人体时的基准帧与起跳/落地帧的对比，通过 MOG2 背景建模 + 轮廓实心填充提取鞋子精确位置，不干预骨骼法的起跳/落地判断逻辑。

```
流程: 基准帧(无人体) + 起跳帧 → MOG2 前景提取 → Otsu 二值化 → 轮廓填充 → 脚尖 X
      基准帧(无人体) + 落地帧 → MOG2 前景提取 → Otsu 二值化 → 轮廓填充 → 脚后跟 X
                                                                              ↓
                                                                        距离 = 落地 - 起跳
```
